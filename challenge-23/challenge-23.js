(function(doc, win){
  ...
  */
  ...
  function isLastItemAnOperations(number) {
    var operations = ['+', '-', 'x', '÷'];
  ...
  function removeLastItemIfItIsAnOperator(number) {
    if(isLastItemAnOperations(number)) {
  ...
  function handleClickEqual() {
    ...
    $visor.value = allValues.reduce(function(accumulated, actual) {
      var firsValue = accumulated.slice(0, -1);
      var operator = accumulated.split('').pop();
      var lastValue = isLastItemAnOperations(actual);
      var lastOperator = isLastItemAnOperations(actual) ? actual.split('').pop() : '';
      switch(operator) {
        case '+':
          return (Number(firsValue) + Number(lastValue)) + lastOperator;
